...
router.get('/test/:id', function(req, res, next) {
    //This code was copied and modified from the Lab 9 Tutorial Source Code
    var MongoClient = require('mongodb').MongoClient;
    var assert = require('assert');
    //Password is confidential because of common use
    var url = 'mongodb://root:<CONFIDENTIAL>@ds227555.mlab.com:27555/lab9';
    var qw = req.params.id;
    var searchDocument = function(db, callback) {

        var finder = db.collection('name').find({"name": qw});
        finder.each(function(err,doc) {
            assert.equal(err,null);

            console.log("Name:" + doc.name);
            console.log("Item:" + doc.item);
            res.render('test', {output: doc.name,output2:doc.item});

        });
    };
    MongoClient.connect(url, function(err, db) {
        assert.equal(null, err);
        searchDocument(db, function() {
            db.close();
        });
    });
